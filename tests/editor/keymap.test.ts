...
describe('bindings', () => {
  ...
  it('binds Alt+Up/Down to line moves', () => {
    expect(find('Alt-ArrowUp')?.run).toBe(moveLinesUp);
    expect(find('Alt-ArrowDown')?.run).toBe(moveLinesDown);
  });

  it('binds Tab / Shift+Tab to indent / outdent', () => {
    expect(find('Tab')?.run).toBe(indentLines);
    expect(find('Tab')?.shift).toBe(outdentLines);
  });

  it('binds Ctrl+/ to line comment toggle and Ctrl+Shift+Up to subtree selection', () => {
    expect(find('Mod-/')?.run).toBe(toggleCommentLines);
...
describe('Alt+Up/Down', () => {
  it('moves a single line', () => {
    const doc = 'a\nb\nc\n';
    expect(run(state(doc, { anchor: at(doc, 'b') }), moveLinesDown).state.doc.toString()).toBe('a\nc\nb\n');
    expect(run(state(doc, { anchor: at(doc, 'b') }), moveLinesUp).state.doc.toString()).toBe('b\na\nc\n');
  });

  it('moves all lines touched by a selection as a block', () => {
    const doc = 'a\nb\nc\nd\n';
    const sel = { anchor: at(doc, 'a'), head: at(doc, 'b') + 1 };
    expect(run(state(doc, sel), moveLinesDown).state.doc.toString()).toBe('c\na\nb\nd\n');
    const sel2 = { anchor: at(doc, 'b'), head: at(doc, 'c') + 1 };
    expect(run(state(doc, sel2), moveLinesUp).state.doc.toString()).toBe('b\nc\na\nd\n');
  });
});
...
describe('Tab / Shift+Tab', () => {
  it('indents the current line by exactly 4 spaces, regardless of cursor column', () => {
    const doc = 'Auth | 2d\n';
    expect(run(state(doc, { anchor: 4 }), indentLines).state.doc.toString()).toBe('    Auth | 2d\n');
  });

  it('indents and outdents every line in a selection', () => {
    const doc = 'a\nb\nc\n';
    const indented = run(state(doc, { anchor: 0, head: at(doc, 'c') + 1 }), indentLines).state;
    expect(indented.doc.toString()).toBe('    a\n    b\n    c\n');
    const back = run(indented, outdentLines).state;
    expect(back.doc.toString()).toBe(doc);
  });

  it('outdents by 4 spaces', () => {
    expect(run(state('        x\n', { anchor: 9 }), outdentLines).state.doc.toString()).toBe('    x\n');
  });

  it('Shift+Tab at column 0 is a no-op', () => {
    const doc = 'Auth | 2d\n    Login | 4h\n';
    const out = run(state(doc, { anchor: 2 }), outdentLines).state;
...
describe('Ctrl+/', () => {
  it('toggles // on the current line and restores it when toggled twice', () => {
    const doc = example;
    const once = run(state(doc, { anchor: at(doc, 'Password') }), toggleCommentLines).state;
    expect(once.doc.line(7).text).toBe('    // Password reset          | 6h  | alice');
    const twice = run(once, toggleCommentLines).state;
    expect(twice.doc.toString()).toBe(doc);
  });

  it('uncomments an existing comment line and re-comments it', () => {
    const doc = example;
    const once = run(state(doc, { anchor: at(doc, '// Q4') }), toggleCommentLines).state;
    expect(once.doc.line(4).text).toBe('Q4 auth work. Estimates are rough.');
    expect(run(once, toggleCommentLines).state.doc.toString()).toBe(doc);
  });

  it('toggles every line in a selection', () => {
    const doc = 'a\n    b\n';
    const once = run(state(doc, { anchor: 0, head: doc.length - 1 }), toggleCommentLines).state;
